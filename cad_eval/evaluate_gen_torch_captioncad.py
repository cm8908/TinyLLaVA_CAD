...
from pythonocc_operator.lib.pc_utils import read_ply
...
def collect_test_set_pcs(args):
    ...
    # all_data = sorted(glob.glob(os.path.join(PC_ROOT, "*/*.ply")))
    all_src_paths = sorted(glob.glob(os.path.join(args.src, "*.ply")))
    # select_idx = random.sample(list(range(len(all_data))), args.n_test + 5)
    # all_data = [all_data[x] for x in select_idx]

    ref_pcs = []
    for src_pc_path in all_src_paths:
    # for pc_path in all_data:
        # pc_path = os.path.join(PC_ROOT, data_id + '.ply')
        data_id = os.path.basename(src_pc_path).split('-')[0]
        truck_id = data_id[:4]
        pc_path = os.path.join(PC_ROOT, truck_id, data_id + '.ply')
        if not os.path.exists(pc_path):
            raise ValueError("File not found: {}".format(pc_path))
        pc = read_ply(pc_path)
...
def main():
    ...
    parser.add_argument("--n_test", type=int, default=207)
    parser.add_argument("--multi", type=int, default=3)
    parser.add_argument("--times", type=int, default=3)
    parser.add_argument("--batch_size", type=int, default=64)
    parser.add_argument("-o", "--output", type=str, help='output log directory', required=True)
    ...
    fp = open(os.path.join(args.output, 'gen_pc_eval.txt'), "w")
